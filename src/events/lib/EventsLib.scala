...
trait Event[+T] {
   ...
   */
  type Sink = (Long, T, ListBuffer[(() => Unit)]) => Unit
 ...
 */
 trait EventNode[T] extends Event[T] {
  ...
  def reactions(id: Long, v: T, reacts: ListBuffer[() => Unit]) {
...
class EventNodeAnd[T1, T2, T](ev1: Event[T1], ev2: Event[T2], merge: (T1, T2) => T) extends EventNode[T] {
  ...
  * Reaction to event1
  */
  lazy val onEvt1 = (id: Long, v1: T1, reacts: ListBuffer[() => Unit]) => {
    if (this.id == id) {
   ...
   * Reaction to event2
   */
  lazy val onEvt2 = (id: Long, v2: T2, reacts: ListBuffer[() => Unit]) => {
    if (this.id == id) {
...
class EventNodeOr[T](ev1: Event[_ <: T], ev2: Event[_ <: T]) extends EventNode[T] {
   ...
   */
  lazy val onEvt = (id: Long, v: T, reacts: ListBuffer[() => Unit]) => {
...
class EventNodeRef[T, U](target: Variable[T], evf: T => Event[U]) extends EventNode[U] {
  ...
  */
   ...
   */
  lazy val onEvt = (id: Long, v: U, reacts: ListBuffer[() => Unit]) => {
    reactions(id, v, reacts)
  }
...
class EventNodeSequence[T, U, V](ev1: Event[T], ev2: => Event[U], merge: (T, U) => V) extends EventNode[V] {
  ...
  * Reaction to event1
  */
  lazy val onEvt1 = (id: Long, v1: T, reacts: ListBuffer[() => Unit]) => {
   ...
   * Reaction to event2
   */
  lazy val onEvt2 = (id: Long, v2: U, reacts: ListBuffer[() => Unit]) => {
...
class EventNodeExcept[T](accpeted: Event[T], except: Event[T]) extends EventNode[T] {
  ...
  lazy val onAccepted = (id: Long, v: T, reacts: ListBuffer[() => Unit]) => {
    myReacts.clear
  ...
  lazy val onExcept = (id: Long, v: T, reacts: ListBuffer[() => Unit]) => {
  ...
  override def reactions(id: Long, v: T, reacts: ListBuffer[() => Unit]) {
    // collect the reactions of this event
